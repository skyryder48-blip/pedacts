...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    lib.print.info('qbx_pedscenarios drug zone persistence initialized.')
end)
...
lib.callback.register('qbx_pedscenarios:server:getDrugZoneState', function(source, zoneId)
    local heat = getHeat(zoneId)
...
lib.callback.register('qbx_pedscenarios:server:rollBuyerSpawn', function(source, zoneId, gameHour)
    local rep = getRep(source, zoneId)
    local heat = getHeat(zoneId)
...
--- Start negotiation
lib.callback.register('qbx_pedscenarios:server:startNegotiation', function(source, zoneId, archetypeId, itemName, quantity)
    local player = exports.qbx_core:GetPlayer(source)
    if not player then return nil end
...
--- Counter-offer
lib.callback.register('qbx_pedscenarios:server:counterOffer', function(source, counterPrice)
...
--- Refuse sale
lib.callback.register('qbx_pedscenarios:server:refuseSale', function(source)
    local session = negotiations[source]
    if session then
        removeRep(source, session.zoneId, Config.Reputation.lossOnRefuseSale)
    end
...
lib.callback.register('qbx_pedscenarios:server:getReputation', function(source, zoneId)
    return getRep(source, zoneId)
end)

--- Get player drug dealing stats (for potential UI)
lib.callback.register('qbx_pedscenarios:server:getDrugStats', function(source, zoneId)
    local rep = getRep(source, zoneId)
