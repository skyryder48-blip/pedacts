...
local function runNegotiation(ped, data)
    ...
    local ok, result = pcall(function()
        ...
        local offer = lib.callback.await('free-trapsales:server:startNegotiation', false,
    ...
    if not ok then
        ...
        pcall(lib.callback.await, 'free-trapsales:server:refuseSale', false)
        return nil
    end
...
local function onInteractWithBuyer(ped)
    ...
    local result = lib.callback.await('free-trapsales:server:completeSale', false, acceptedPrice)
...
local function startUndercoverDispatch(driverPed, zoneId)
    ...
    TriggerServerEvent('free-trapsales:server:sendUndercoverDispatch', zoneId)
...
local function handleRobberyBehavior(driverPed)
    ...
    lib.callback.await('free-trapsales:server:processVehicleRobbery', false, data.zoneId)
...
local function handleSupplierInteraction(driverPed)
    ...
    local inventory = lib.callback.await('free-trapsales:server:getSupplierInventory', false, data.zoneId)
    ...
    for _, offer in ipairs(inventory.items) do
        options[#options + 1] = {
            ...
            icon = 'fas fa-box',
            onSelect = function()
                local result = lib.callback.await('free-trapsales:server:processSupplierSale', false,
...
local function onInteractWithVehicleBuyer(driverPed)
    ...
    local result = lib.callback.await('free-trapsales:server:completeSale', false, acceptedPrice, data.archetypeId)
...
spawnVehicleBuyer = function(zoneConfig)
    ...
    local gameHour = GetClockHours()
    local spawnData = lib.callback.await('free-trapsales:server:rollVehicleBuyerSpawn', false,
        zoneConfig.id, gameHour)
